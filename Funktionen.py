...
def find_gemcitabin(s):
    gemcitabin_pattern = (
        r"Gemcibatin|Gemcibatine|Gemcibatine Mono|Gemcibatin Mono"
    )
...
def preprocessing_func(input_col, split_string = True, split_pattern = r"[;,]"):
    ...
    if split_string:
        if split_pattern is None:
            raise ValueError("Bitte gibt ein gültiges split_pattern ein oder setze split_sting auf False")
        try:
            re.compile(split_pattern)
        except re.error as e:
            raise ValueError(f"Das split_pattern {split_pattern} ist ungültig Error: {e}")
    ...
    remove_patterns = re.compile(r'\b(?:o\.n\.a\.|o\.n\.a|mg|kg|i\.v\.|i\.v)\b', re.IGNORECASE)
...
def find_matches(processed_df, reference_df, fuzzy_threshold=90):
    ...
    for text in processed_df["Processed"]:
        ...
        # Exakte Treffer
        exact_matches = [word for word in reference_words if word.lower() == text.lower()]
        exact_matches_list.append(exact_matches)

        # Fuzzy Treffer, wenn keine exakten Treffer gefunden
        if not exact_matches:
            fuzzy_matches = [
                word for word in reference_words if fuzz.partial_ratio(text, word) >= fuzzy_threshold
            ]
    ...
    max_exact_matches = max(len(matches) for matches in exact_matches_list) if exact_matches_list else 0
    max_fuzzy_matches = max(len(matches) for matches in fuzzy_matches_list) if fuzzy_matches_list else 0
    ...
    for i in range(max_exact_matches):
        ...
        exact_match_columns[column_name] = [matches[i] if i < len(matches) else "" for matches in exact_matches_list]

    for i in range(max_fuzzy_matches):
        column_name = f"FuzzyMatch{i+1}"
        fuzzy_match_columns[column_name] = [matches[i] if i < len(matches) else "" for matches in fuzzy_matches_list]
...
def calculate_best_match(processed_df, reference_df, split_string = True):
    ...
    for _, row in processed_df.iterrows():
        ...
        match_columns = [col for col in processed_df.columns if col.startswith("Match") or col.startswith("FuzzyMatch")]
        
        distances = {}
        for col in match_columns:
            match_word = row[col]
            if match_word:  
                dist = calculate_similarity_percentage(processed_text.lower(), match_word.lower())
    ...
    if split_string:
        collapsed_df = processed_df.groupby("ID").agg({
            "Original": "first",  
            "Processed": lambda x: "; ".join(map(str, filter(None, x.fillna("")))),  
            "Best_match": lambda x: "; ".join(dict.fromkeys(map(str, filter(None, x.fillna(""))))) 
        }).reset_index()
    else:
